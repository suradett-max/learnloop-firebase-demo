...
function usernameEmail(username) {
  const encoded = [...String(username).trim().toLowerCase()].map((ch) => ch.codePointAt(0).toString(16)).join("-");
  return `${encoded}@classroom.wrk.local`;
}
...
function renderDashboard() {
  ...
  const atRisk = students.filter((s) => ["absent", "late", "leave"].includes(currentAttendance[s.id])).slice(0, 5);
  $("#riskCount").textContent = `${atRisk.length} คน`;
  $("#riskStudents").innerHTML = atRisk.length ? atRisk.map((s) => `<div class="insight-row"><b>${escapeHtml(s.number)}</b><span><strong>${escapeHtml(s.name)}</strong><small>ม.${escapeHtml(String(s.grade).replace("ม.", ""))}/${escapeHtml(s.room)} · ${currentAttendance[s.id] === "absent" ? "ขาดเรียน" : currentAttendance[s.id] === "late" ? "มาสาย" : "ลา"}</small></span></div>`).join("") : `<div class="empty"><p>ยังไม่มีรายการที่ต้องติดตามในห้องที่เลือก</p></div>`;
  $("#recentActivity").innerHTML = [`เช็กชื่อ ${Object.keys(currentAttendance).length} คน`, `มีงานในระบบ ${assignments.length} รายการ`, `ดูแล ${classes.length} ชั้นเรียน`].map((text, i) => `<div class="timeline-row"><b>${["✓","☆","▦"][i]}</b><span><strong>${text}</strong><small>${i === 0 ? "ข้อมูลประจำวันที่เลือก" : "ภาคเรียน 1/2569"}</small></span></div>`).join("");
  const submitted = Object.values(currentScores).filter((v) => v?.status === "submitted" || v?.submitted).length;
  $("#gradebookHealth").innerHTML = `<div class="health-row"><span>งานทั้งหมด</span><strong>${assignments.length}</strong></div><div class="health-row"><span>บันทึกคะแนนแล้ว</span><strong>${submitted}</strong></div><div class="health-row"><span>รายวิชา</span><strong>${subjects.length}</strong></div>`;
}

function renderProfile() {
  $("#profileName").textContent = profile.displayName || profile.firstName || "ผู้ใช้งาน";
  $("#profileRole").textContent = profile.role === "admin" ? "ผู้ดูแลระบบ" : (profile.position || "ครูผู้สอน");
...
async function loadAttendance() {
  ...
  currentAttendanceNotes = record?.data().notes || {};
  ...
  $("#attendanceList").innerHTML = list.map((s) => `<div class="student-attendance" data-student="${s.id}">
    ...
    <input class="attendance-note" value="${escapeHtml(currentAttendanceNotes[s.id] || "")}" placeholder="หมายเหตุรายบุคคล เช่น ป่วย มีใบลา หรือมาสายเพราะ...">
  </div>`).join("");
...
function renderScores() {
  const subject = $("#scoreSubject").value;
  const room = $("#scoreRoom").value;
  const roomAssignments = assignments.filter((a) => a.subject === subject && String(a.room) === room);
  const list = studentsInRoom($("#scoreRoom").value);
  const table = $("#scoreRows").closest("table");
  table.className = "gradebook"; table.parentElement.className = "gradebook-wrap";
  table.querySelector("thead").innerHTML = `<tr><th>เลขที่</th><th>ชื่อ-นามสกุล</th>${roomAssignments.map((a) => `<th class="assignment-column" title="${escapeHtml(a.description || a.title)}">${escapeHtml(a.title)}<small>เต็ม ${a.maxScore}</small></th>`).join("")}<th>หมายเหตุ</th></tr>`;
  $("#scoreRows").innerHTML = roomAssignments.length ? list.map((s) => `<tr data-score-student="${s.id}"><td>${escapeHtml(s.number)}</td><td><strong>${escapeHtml(s.name)}</strong><small>${escapeHtml(s.studentId)}</small></td>${roomAssignments.map((a) => { const value = currentScores[a.id]?.[s.id] || {}; return `<td class="assignment-column" data-assignment-cell="${a.id}">${a.type === "Check" ? `<input class="matrix-check" type="checkbox" ${value.submitted || value.status === "submitted" ? "checked" : ""} aria-label="${escapeHtml(a.title)} ${escapeHtml(s.name)}">` : `<input class="matrix-score" type="number" min="0" max="${a.maxScore}" value="${value.score ?? ""}" placeholder="/${a.maxScore}" aria-label="${escapeHtml(a.title)} ${escapeHtml(s.name)}">`}</td>`; }).join("")}<td><input class="note-cell" value="${escapeHtml(roomAssignments.map((a)=>currentScores[a.id]?.[s.id]?.note).find(Boolean)||"")}" placeholder="หมายเหตุ"></td></tr>`).join("") : "";
  if (!table.parentElement.querySelector(".paste-hint")) table.parentElement.insertAdjacentHTML("beforebegin", `<p class="paste-hint">เคล็ดลับ: เลือกช่องคะแนนช่องแรกแล้ววางข้อมูลหลายแถวจาก Excel ได้ทันที · ใช้ปุ่มลูกศรขึ้น/ลงเพื่อเลื่อนระหว่างนักเรียน</p>`);
  $("#scoreEmpty").hidden = Boolean(roomAssignments.length && list.length);
}

async function loadScores() {
  if (!user) { currentScores = {}; renderScores(); return; }
  const snapshot = await getDocs(query(refs().scores, limit(250)));
  currentScores = Object.fromEntries(snapshot.docs.map((d) => [d.id, d.data().results || {}]));
  renderAssignments();
}
...
function preparePrint() {
  ...
  else if (active === "attendance") { title = "แบบบันทึกการมาเรียน"; subtitle = `${$("#attendanceSubject option:checked").textContent} · ม.2/${$("#attendanceRoom").value} · ${formatDate($("#attendanceDate").value)}`; const labels={present:"มา",late:"สาย",leave:"ลา",absent:"ขาด"}; content=`<table><thead><tr><th>เลขที่</th><th>รหัส</th><th>ชื่อ-นามสกุล</th><th>สถานะ</th><th>หมายเหตุ</th></tr></thead><tbody>${studentsInRoom($("#attendanceRoom").value).map((s)=>`<tr><td>${s.number}</td><td>${s.studentId}</td><td>${escapeHtml(s.name)}</td><td>${labels[currentAttendance[s.id]]||"-"}</td><td>${escapeHtml(currentAttendanceNotes[s.id]||"")}</td></tr>`).join("")}</tbody></table>`; }
  else if (active === "scores") { const room=$("#scoreRoom").value; const list=assignments.filter((a)=>a.subject===$("#scoreSubject").value&&String(a.room)===room); title="แบบบันทึกงานและคะแนน"; subtitle=`${$("#scoreSubject option:checked").textContent} · ม.2/${room} · ภาคเรียน 1/2569`; content=`<table><thead><tr><th>เลขที่</th><th>รหัส</th><th>ชื่อ-นามสกุล</th>${list.map((a)=>`<th>${escapeHtml(a.title)}<br>(${a.maxScore})</th>`).join("")}<th>รวม</th></tr></thead><tbody>${studentsInRoom(room).map((s)=>{let total=0;return `<tr><td>${s.number}</td><td>${s.studentId}</td><td>${escapeHtml(s.name)}</td>${list.map((a)=>{const v=currentScores[a.id]?.[s.id]||{};const shown=a.type==="Check"?(v.submitted||v.status==="submitted"?"✓":"-"):(v.score??"");total+=Number(v.score)||0;return `<td>${shown}</td>`}).join("")}<td>${total}</td></tr>`}).join("")}</tbody></table>`; }
...
function bindEvents() {
  $("#collapseSidebar").addEventListener("click", () => { document.body.classList.toggle("sidebar-collapsed"); localStorage.setItem("sidebarCollapsed", document.body.classList.contains("sidebar-collapsed") ? "1" : "0"); });
  $("#logoutButton").addEventListener("click", () => signOut(auth));
  $("#loginForm").addEventListener("submit", async (e) => { e.preventDefault(); const button=e.submitter; const message=$("#loginMessage"); button.disabled=true; message.textContent="กำลังตรวจสอบบัญชี..."; try { await signInWithEmailAndPassword(auth, usernameEmail($("#loginUsername").value), $("#loginPassword").value); e.currentTarget.reset(); message.textContent=""; } catch (error) { message.textContent = error.code === "auth/invalid-credential" ? "ชื่อผู้ใช้หรือรหัสผ่านไม่ถูกต้อง" : `เข้าสู่ระบบไม่สำเร็จ: ${error.code}`; } finally { button.disabled=false; } });
  ...
  $("#attendanceList").addEventListener("change", (e) => { const row = e.target.closest("[data-student]"); if (!row) return; if (e.target.classList.contains("attendance-note")) currentAttendanceNotes[row.dataset.student] = e.target.value.trim(); else { currentAttendance[row.dataset.student] = e.target.value; updateAttendanceSummary(); } });
  ...
  $("#saveAttendance").addEventListener("click", async () => { await setDoc(doc(refs().attendance, attendanceDocId()), { date: $("#attendanceDate").value, subject: $("#attendanceSubject").value, room: $("#attendanceRoom").value, statuses: currentAttendance, notes: currentAttendanceNotes, updatedAt: serverTimestamp() }); toast("บันทึกการเช็กชื่อและหมายเหตุแล้ว"); });
  $("#clearAttendance").addEventListener("click", async () => { if (confirm("ลบบันทึกการเช็กชื่อของวันที่ วิชา และห้องที่เลือกหรือไม่?")) { await deleteDoc(doc(refs().attendance, attendanceDocId())); currentAttendance = {}; currentAttendanceNotes = {}; studentsInRoom($("#attendanceRoom").value).forEach((s) => { currentAttendance[s.id] = "present"; }); loadAttendance(); toast("ล้างบันทึกการเช็กชื่อแล้ว"); } });
  $("#saveScores").addEventListener("click", async () => { const roomAssignments = assignments.filter((a) => a.subject === $("#scoreSubject").value && String(a.room) === $("#scoreRoom").value); if (!roomAssignments.length) return toast("ยังไม่มีงานในห้องนี้", true); const batch = writeBatch(db); roomAssignments.forEach((assignment) => { const results = {}; $$("[data-score-student]").forEach((row) => { const cell = row.querySelector(`[data-assignment-cell="${CSS.escape(assignment.id)}"]`); const check = cell?.querySelector(".matrix-check"); const score = cell?.querySelector(".matrix-score"); results[row.dataset.scoreStudent] = { status: check ? (check.checked ? "submitted" : "pending") : (score?.value === "" ? "pending" : "submitted"), submitted: check ? check.checked : score?.value !== "", score: score?.value === "" || !score ? null : Number(score.value), note: row.querySelector(".note-cell")?.value.trim() || "" }; }); batch.set(doc(refs().scores, assignment.id), { assignmentId: assignment.id, results, updatedAt: serverTimestamp() }, { merge: true }); currentScores[assignment.id] = results; }); await batch.commit(); toast("บันทึกคะแนนทุกคอลัมน์แล้ว"); renderDashboard(); });
  $("#scoreRows").addEventListener("keydown", (e) => { if (!e.target.matches(".matrix-score")) return; const inputs = $$(".matrix-score"); const index = inputs.indexOf(e.target); if (e.key === "ArrowDown" || e.key === "ArrowUp") { e.preventDefault(); inputs[index + (e.key === "ArrowDown" ? 1 : -1)]?.focus(); } });
  $("#scoreRows").addEventListener("paste", (e) => { if (!e.target.matches(".matrix-score")) return; const rows = e.clipboardData.getData("text").trim().split(/\r?\n/).map((line) => line.split("\t")); if (rows.length < 2 && rows[0].length < 2) return; e.preventDefault(); const allRows = $$("[data-score-student]"); const startRow = allRows.indexOf(e.target.closest("tr")); const startCell = [...e.target.closest("tr").querySelectorAll(".matrix-score")].indexOf(e.target); rows.forEach((values, r) => values.forEach((value, c) => { const input = allRows[startRow + r]?.querySelectorAll(".matrix-score")[startCell + c]; if (input) input.value = value.trim(); })); toast(`วางคะแนนจาก Excel แล้ว ${rows.length} แถว`); });
...
function subscribe() {
  ...
  unsubscribers = [
    onSnapshot(query(refs().students, orderBy("number"), limit(4000)), (snap) => { students = snap.docs.map((d) => ({ id: d.id, ...d.data() })); updateRoomOptions(); renderStudents(); loadAttendance(); }),
...
onAuthStateChanged(auth, (account) => {
  if (account?.isAnonymous) { signOut(auth); return; }
  user = account;
  $("#loginScreen").classList.toggle("hidden", Boolean(account));
  if (account) subscribe();
  else { unsubscribers.forEach((fn) => fn()); unsubscribers=[]; students=[]; assignments=[]; classes=[]; currentAttendance={}; currentScores={}; $("#syncStatus").classList.remove("online"); $("#syncStatus").lastChild.textContent=" รอเข้าสู่ระบบ"; }
});
